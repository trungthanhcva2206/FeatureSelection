"""
Variance Threshold selector using scikit-learn.
...
def select_by_top_k(variances: np.ndarray, k: int) -> np.ndarray:
...
def run(
    ...
    if cv_topk_grid:
        ...
        for k in cv_topk_grid:
            ...
            mask_k = select_by_top_k(variances, k)
            X_k = X[:, mask_k]
    ...
    report_lines.append(f"Selection mode: {mode} | normalize={normalize}")
    ...
    top_order = np.argsort(variances)[::-1][:5]
    top_preview = [(feature_names[i], variances[i]) for i in top_order]
    report_lines.append("")
    report_lines.append("Top 5 variances (name, variance):")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Variance Threshold selector using scikit-learn"
    )
    ...
    parser.add_argument(
        "--top-k",
        type=int,
        help="Select the top-k features by variance",
    )
    parser.add_argument(
        "--normalize",
        action="store_true",
        help="Normalize features to variance of normalized data",
    )
    parser.add_argument(
        "--log-file",
        type=Path,
