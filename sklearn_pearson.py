"""
Pearson Correlation selector using scikit-learn.
- Computes Pearson correlation using pandas/numpy with sklearn-like interface.
...
def pearson_scores(X: np.ndarray, y: np.ndarray, use_abs: bool = True) -> np.ndarray:
    """Compute Pearson correlation per feature using numpy."""
    ...
    for j in range(n_features):
        ...
        if col_std < 1e-12:
            ...
            score = float(np.mean(col_centered * y_centered) / (col_std * y_std))
...
def run(
    ...
    corr_scores = pearson_scores(X, y, use_abs=use_abs)
    ...
    if cv_topk_grid:
        ...
        for k in cv_topk_grid:
            ...
            mask_k = select_by_top_k(corr_scores, k)
            X_k = X[:, mask_k]
    ...
    if top_k is not None:
        mask = select_by_top_k(corr_scores, top_k)
    else:
    ...
    report_lines.append(f"Selection mode: {mode} | use_abs={use_abs}")
    ...
    top_order = np.argsort(corr_scores)[::-1][:5]
    top_preview = [(feature_names[i], corr_scores[i]) for i in top_order]
    report_lines.append("")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Pearson Correlation selector")
    parser.add_argument("csv", type=Path, help="CSV file with label in first column")
    parser.add_argument(
        "--threshold",
        type=float,
        default=0.0,
        help="Keep features whose (abs) correlation is >= threshold",
    )
    parser.add_argument(
        "--top-k",
        type=int,
