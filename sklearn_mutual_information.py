"""
Mutual Information selector using scikit-learn.
- Computes mutual information using sklearn.feature_selection.mutual_info_classif.
...
def run(
    ...
    if cv_topk_grid:
        ...
        for k in cv_topk_grid:
            ...
            mask_k = select_by_top_k(mi_scores, k)
            X_k = X[:, mask_k]
    ...
    if top_k is not None:
        mask = select_by_top_k(mi_scores, top_k)
    else:
    ...
    top_order = np.argsort(mi_scores)[::-1][:5]
    top_preview = [(feature_names[i], mi_scores[i]) for i in top_order]
    report_lines.append("")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Mutual Information selector using scikit-learn"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Keep features whose MI score is >= threshold",
    )
    parser.add_argument(
        "--top-k",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Number of neighbors for MI estimation",
    )
    parser.add_argument(
        "--log-file",
        type=Path,
