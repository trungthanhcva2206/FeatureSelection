"""
Manual Pearson Correlation feature selector.
- Assumes the first column of the CSV is the label (numeric or encoded categorical).
- Computes per-feature Pearson correlation with the label.
...
def pearson_scores(
    ...
    """Compute Pearson correlation per feature; return absolute value if use_abs.
    Constant columns or zero-variance label yield score 0.
    """
    ...
    if y_var < epsilon:
        return scores  # label is constant

    for j in range(n_features):
        col = X[:, j]
        col_mean = col.mean()
        col_centered = col - col_mean
        denom = np.sqrt(np.sum(col_centered ** 2) * y_var)
        if denom < epsilon:
            score = 0.0
        else:
            score = float(np.sum(col_centered * y_centered) / denom)
...
def run(
    ...
    scores = pearson_scores(X, y, use_abs=use_abs, epsilon=epsilon)
    ...
    stats = CorrStats(correlations=scores, used_absolute=use_abs, epsilon=epsilon)
    result = CorrResult(
        selected_mask=mask,
    ...
    report_lines.append(
        f"Selection mode: {mode} | use_abs={use_abs} | epsilon={epsilon}"
    )
    report_lines.append(
    ...
    report_lines.append("")
    report_lines.append("Top 5 correlation scores (name, score):")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Manual Pearson Correlation feature selector")
    parser.add_argument("csv", type=Path, help="CSV file with label in first column")
    parser.add_argument(
        "--threshold",
        type=float,
        default=0.0,
        help="Keep features whose (abs) correlation is >= threshold (ignored if top-k is set)",
    )
    parser.add_argument(
        "--top-k",
        type=int,
        help="Select the top-k features by correlation score",
    )
    parser.add_argument(
        "--use-abs",
        action="store_true",
        help="Use absolute correlation (default true). If omitted, signed correlation is kept.",
    )
    parser.add_argument(
        "--no-abs",
        action="store_true",
        help="Use signed correlation instead of absolute value.",
    )
    parser.add_argument(
        "--epsilon",
        type=float,
        default=1e-12,
        help="Small value to avoid division by zero when variance is tiny",
    )
    parser.add_argument(
        "--log-file",
        type=Path,
    ...
    # Determine absolute vs signed correlation
    use_abs = True
    if args.no_abs:
        use_abs = False
    if args.use_abs:
