"""
Block-wise Ridge feature scoring then SVM training.
- Shuffle features, split into blocks.
- For each block: cross-validate Ridge to score features, choose best k per block.
...
def ridge_block_select(
    ...
    for b in range(n_blocks):
        ...
        # Find best alpha for this block
        best_score = -np.inf
        ...
        # Fit on full block with best alpha to rank features
        pipe_full = Pipeline(
            [
                ("scaler", StandardScaler()),
                ("ridge", RidgeClassifier(alpha=best_alpha, class_weight=class_weight)),
            ]
        )
        pipe_full.fit(X_block, y)
        coefs = pipe_full.named_steps["ridge"].coef_
        coef_mag = np.max(np.abs(coefs), axis=0) if coefs.ndim > 1 else np.abs(coefs)
    ...
    return RidgeBlockResult(selected_indices=selected, block_scores=block_scores)
...
def run(
    ...
    ridge_result = ridge_block_select(
        X_train,
        y_train,
    ...
    union = sorted(set(ridge_result.selected_indices))
    if not union:
        union = [int(perm[0])]
    ...
    report_lines = [
        ...
        f"Ridge alpha grid: {alpha_grid} | k grid per block: {k_grid}",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Block-wise Ridge feature scoring + SVM")
    ...
    parser.add_argument("--alpha-grid", type=str, default="0.1,1,5,10", help="Comma-separated alpha grid for Ridge")
    ...
    parser.add_argument("--class-weight-balanced", action="store_true", help="Use class_weight='balanced' for Ridge and SVM")
